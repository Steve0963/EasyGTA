...
def read_memory(address,data_type=ctypes.c_ulong):
    ...
    buffer = ctypes.create_string_buffer(buffer_size)
    bytes_read = ctypes.c_uint64()
    ctypes.windll.kernel32.ReadProcessMemory(process, ctypes.c_uint64(address), buffer, buffer_size, ctypes.byref(bytes_read))
    ctypes.windll.kernel32.CloseHandle(process)
...
def write_memory(address,type,data):
    ...
    if process:
        ...
        success = ctypes.windll.kernel32.WriteProcessMemory(process, ctypes.c_uint64(address), buffer, buffer_size,ctypes.byref(bytes_written))
        ctypes.windll.kernel32.CloseHandle(process)
        return success != 0
    else:
        return False  